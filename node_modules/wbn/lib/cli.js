...
import { BundleBuilder, combineHeadersForUrl, isHeaders, } from './encoder.js';
import * as fs from 'fs';
import * as path from 'path';
import mime from 'mime';
import { APPROVED_VERSIONS, B1, B2, DEFAULT_VERSION, } from './constants.js';
export function addFile(builder, url, file, overrideHeaders) {
    const headers = {
        'Content-Type': mime.getType(file) || 'application/octet-stream',
    };
    builder.addExchange(url, 200, combineHeadersForUrl(headers, overrideHeaders, url), fs.readFileSync(file));
}
export function addFilesRecursively(builder, baseURL, dir, overrideHeaders) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(filePath).isDirectory()) {
            addFilesRecursively(builder, baseURL + file + '/', filePath, overrideHeaders);
        }
            ...
            builder.addExchange(baseURL + file, 301, combineHeadersForUrl({ Location: './' }, overrideHeaders, baseURL + file), '');
        }
        else {
...
function readOptions() {
    return commander
        ...
        .option('-f, --formatVersion <formatVersion>', 'webbundle format version, possible values are ' +
        APPROVED_VERSIONS.map((v) => `"${v}"`).join(' and ') +
        ' (default: "' +
        DEFAULT_VERSION +
        '")', DEFAULT_VERSION)
        .option('-p, --primaryURL <URL>', 'primary URL (defaults to base URL, only valid with format version "' +
        B1 +
        '")')
        ...
        .option('-h, --headerOverride <jsonFilePath>', 'path to a JSON file specifying the headers as an object of strings')
        .parse(process.argv);
}
function validateOptions(options) {
    ...
    if (options.formatVersion !== undefined &&
        ...
        return ('error: invalid format version (must be ' +
            APPROVED_VERSIONS.map((val) => `"${val}"`).join(' or ') +
            ').');
    }
    if (options.formatVersion === B1 &&
        !(options.primaryURL || options.baseURL)) {
        return 'error: Primary URL is required.';
    }
    return null;
}
function readHeaderOverridesFile(path) {
    try {
...
export function main() {
    ...
    const version = options.formatVersion === undefined
        ? DEFAULT_VERSION
        : options.formatVersion;
    const headerOverrides = options.headerOverride === undefined
        ? undefined
    ...
    addFilesRecursively(builder, options.baseURL || '', options.dir, headerOverrides);
